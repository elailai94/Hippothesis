/*
 * Copyright 2017-present, Hippothesis, Inc.
 * All rights reserved.
 *
 * This source code is licensed under the BSD-style license found in the
 * LICENSE file in the root directory of this source tree.
 */

'use strict';

const ActionTypes = {
  navigation: {
    SELECT_RECIPE: 'SELECT_RECIPE'
  },
  recipeSuggestionResults: {
    SUGGEST_RECIPES_REQUEST: 'SUGGEST_RECIPES_REQUEST',
    SUGGEST_RECIPES_SUCCESS: 'SUGGEST_RECIPES_SUCCESS',
    SUGGEST_RECIPES_FAILURE: 'SUGGEST_RECIPES_FAILURE'
  },
  recipeSearchResults: {
    SEARCH_RECIPES_REQUEST: 'SEARCH_RECIPES_REQUEST',
    SEARCH_RECIPES_SUCCESS: 'SEARCH_RECIPES_SUCCESS',
    SEARCH_RECIPES_FAILURE: 'SEARCH_RECIPES_FAILURE'
  },
  recipes: {
    ADD_RECIPE: 'ADD_RECIPE_TO_RECIPES',
    ADD_RECIPES: 'ADD_RECIPES_TO_RECIPES'
  },
  recipeSearchIngredientsList: {
    ADD_INGREDIENT: 'ADD_INGREDIENT_TO_RECIPE_SEARCH_INGREDIENTS_LIST',
    REMOVE_INGREDIENT: 'REMOVE_INGREDIENT_FROM_RECIPE_SEARCH_INGREDIENTS_LIST',
    EDIT_INGREDIENT: 'EDIT_INGREDIENT_IN_RECIPE_SEARCH_INGREDIENTS_LIST'
  },
  shoppingList: {
    ADD_INGREDIENT: 'ADD_INGREDIENT_TO_SHOPPING_LIST',
    REMOVE_INGREDIENT: 'REMOVE_INGREDIENT_FROM_SHOPPING_LIST',
    EDIT_INGREDIENT: 'EDIT_INGREDIENT_IN_SHOPPING_LIST',
    MARK_INGREDIENT_AS_BOUGHT: 'MARK_INGREDIENT_AS_BOUGHT_IN_SHOPPING_LIST',
    MARK_INGREDIENT_AS_NOT_BOUGHT: 'MARK_INGREDIENT_AS_NOT_BOUGHT_IN_SHOPPING_LIST'
  },
  inventoryList: {
    ADD_INGREDIENT: 'ADD_INGREDIENT_TO_INVENTORY_LIST',
    REMOVE_INGREDIENT: 'REMOVE_INGREDIENT_FROM_INVENTORY_LIST',
    EDIT_INGREDIENT:'EDIT_INGREDIENT_IN_INVENTORY_LIST',
    MARK_INGREDIENT_AS_USED: 'MARK_INGREDIENT_AS_USED_IN_INVENTORY_LIST',
    MARK_INGREDIENT_AS_NOT_USED: 'MARK_INGREDIENT_AS_NOT_USED_IN_INVENTORY_LIST'
  },
  filters: {
    UPDATE_ALLERGIES: 'UPDATE_ALLERGIES',
    UPDATE_CUISINES: 'UPDATE_CUISINES',
    UPDATE_DIETS: 'UPDATE_DIETS',
    UPDATE_NUTRITION: 'UPDATE_NUTRITION',
    UPDATE_TYPES: 'UPDATE_TYPES'
  },
  recipesStore: {
    ADD_RECIPE_TO_STORE: 'ADD_RECIPE_TO_STORE',
    ADD_RECIPES_TO_STORE: 'ADD_RECIPES_TO_STORE'    
  },
  dietaryPreferences: {
    ADD_ALLERGY: 'ADD_ALLERGY',
    REMOVE_ALLERGY: 'REMOVE_ALLERGY',
    REMOVE_ALL_ALLERGIES: 'REMOVE_ALL_ALLERGIES',
    ADD_CUISINE: 'ADD_CUISINE',
    REMOVE_CUISINE: 'REMOVE_CUISINE',
    REMOVE_ALL_CUISINES: 'REMOVE_ALL_CUISINES',
    ADD_DIET: 'ADD_DIET',
    REMOVE_DIET: 'REMOVE_DIET',
    REMOVE_ALL_DIETS: 'REMOVE_ALL_DIETS',
    ADD_DISLIKED_INGREDIENT: 'ADD_DISLIKED_INGREDIENT',
    REMOVE_DISLIKED_INGREDIENT: 'REMOVE_DISLIKED_INGREDIENT',
    REMOVE_ALL_DISLIKED_INGREDIENTS: 'REMOVE_ALL_DISLIKED_INGREDIENTS',
    EDIT_DISLIKED_INGREDIENT: 'EDIT_DISLIKED_INGREDIENT',
  }
};

export default ActionTypes;
